...
Status: `OWNER_DECISION_REQUIRED_CONTENT_NOT_AUTHORIZED`

## 目的

承認済み R7 schema を使って具体的攻略を作り始める前に、初期 authoring の件数、対象、証拠、
review 境界を固定する。本 packet は decision-only であり、現時点では具体的 route を1件も含まず、
`r7-pilot-content-authoring` も `not-authorized` のままである。
...
- current route artifact count: `0`
...
- request、schema、空 catalog、R6 approval packet の digest が変わった場合は再審査する。
...
- content gate が承認されるまでは catalog を空のまま維持する。
