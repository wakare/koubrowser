...
Status: `R7_REAL_ACCOUNT_ACCEPTANCE_REVISION_3_AUTHORIZED_NOT_RUN`
...
`REAL_ACCOUNT_READONLY_ACCEPTANCE_AUTHORIZED`、revision 3 execution は `authorized` である。
承認範囲は固定2 route の1回の只読・脱敏再実行に限る。
