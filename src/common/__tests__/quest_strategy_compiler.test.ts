...
describe('quest strategy runtime v2 compiler', () => {
  it('reproduces the checked-in runtime artifacts', () => {
    ...
    expect(output).toContain('27 facts, 39 routes, 60 stage contributions, 0 rejected objectives')
  })
  ...
  it('compiles exact stage contributions without promoting partial multi-stage quests', () => {
    ...
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-5-1-surface-striking-monthly')
        ?.contributions
    ).toMatchObject([{ questId: 259, stageIndex: 0, mapKey: '5-1' }])
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-5-4-31st-destroyer-quarterly')
        ?.contributions
    ).toMatchObject([{ questId: 875, stageIndex: 0, mapKey: '5-4' }])
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-5-1-new-mikawa-quarterly')
        ?.contributions
    ).toMatchObject([{ questId: 888, stageIndex: 0, mapKey: '5-1' }])
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-5-3-new-mikawa-quarterly')
        ?.contributions
    ).toMatchObject([{ questId: 888, stageIndex: 1, mapKey: '5-3' }])
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-5-4-new-mikawa-quarterly')
        ?.contributions
    ).toMatchObject([{ questId: 888, stageIndex: 2, mapKey: '5-4' }])
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-5-1-sixth-squadron-quarterly')
        ?.contributions
    ).toMatchObject([{ questId: 903, stageIndex: 0, mapKey: '5-1' }])
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-5-4-sixth-squadron-quarterly')
        ?.contributions
    ).toMatchObject([{ questId: 903, stageIndex: 1, mapKey: '5-4' }])
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-6-4-sixth-squadron-quarterly')
        ?.contributions
    ).toMatchObject([{ questId: 903, stageIndex: 2, mapKey: '6-4' }])
    expect(
      bundle.routes.find((route) => route.routeId === 'normal-6-5-sixth-squadron-quarterly')
        ?.contributions
    ).toMatchObject([{ questId: 903, stageIndex: 3, mapKey: '6-5' }])
    expect(
  ...
  it('rejects opaque hard constraints and emits withdrawal dependencies', () => {
    ...
    expect(withdrawals.dependencies).toHaveLength(39)
  })
